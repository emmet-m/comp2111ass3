...
\textbf{Deq:} \\
$q : [|xs| > 0 \land q = (N, sx), q = (N, s) \land s = s_0 ] \refinedby  deq(q)$ \\
\textbf{WhosNext:} \\
...
%
%
\rc{a-ass, ass} \explain{To re-establish our coupling invariant, we must update out array at position $n$ to match the corresponding element in the sequence (as described in the first conjunct) and then increment $n$ to ensure that $|s| = (n - m)(mod N + 1)$ (as described in the second conjunct).} \\
\textbf{con } S $\cdot$ enq(q,x);\\
A[n] := x; n := n + 1 \\ \\
%
%
\textcolor{green}{(C)}: \\
%
$ q, A, m, n : [\C((N,sy),A,m,n) \land |sy| \geq 0, (\C((N,s),A,m,n)) \land s= s_0 ];$
%
\textcolor{green}{}
%
\subsection{Step 4}
