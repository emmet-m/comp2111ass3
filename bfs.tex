...
\textbf{Enq:} \\
$q : [n <= N, ~ q = ( N, xs_0) \land |s| \neq 0] \refinedby enq(q, x)$ \\
\textbf{Deq:} \\
$q : [n > 0 \land q = (N, xs), q = (N, s_0) ] \refinedby  deq(q)$ \\
\textbf{WhosNext:} \\
...
%
(2) \rc{seq} \explain{We introduce a constant to freeze the value of $n$} \\
$ \null \quad v, f, q : \left[ 	
\begin{array}{l}
	pre(1) \land q = (N,s) \land |s| = 0,  \\
	pre(1) \land q = (N,xs) \land |s| \neq 0 \land x = r_t
\end{array}
\right]$; \\
$ \tabb v, f, q : \left[ 	
\begin{array}{l}
...
We are able to represent the queue's set directly as the elements in the same level as the current node and the descendants of the current node being explored as it is a precondition that all nodes have a unique parent, implying that no node can be added multiple times. This is cemented by the lack of cycles in the tree. \\ \\
(6) \rc{i-loc, seq x 2} \explain{We split up our statement so we can perform deq() and whosNext()} \\ \\
$
\null \quad \textbf{var } t \cdot 
\null \quad v, f, q, t : \left[ pre(1) \land q = (N, sy), pre(1) \land q = (N, sy) \land t = y \right] \\
\null \quad v, f, q, t : \left[ pre(1) \land q = (N, sy) \land t = y, pre(1) \land q = (N, s) \land t = y \land n = n_0 - 1 \right] \\ %TODO
\null \quad v, f, q, t : \left[ pre(6) \land t = s_n, post(6) \right]
$ \\ \\
...
t := whosNext(q); \\
\tabb deq(q); \\ 
\null \quad \nt{v, f, q, t : \left[ pre(6) \land t = s_n, post(6) \right]}{(7)}
$ \\ \\
...
\begin{algorithm}[H]
\eIf { $\kappa_t(t) = k$ } {
$\nt{v, f, q, t : \left[ pre(6) \land t = s_n \land \kappa_t(t) = k, post(6) \right]}{(8)}$
} {
$\nt{v, f, q, t : \left[ pre(6) \land t = s_n \land \kappa_t(t) \neq k, post(6) \right]}{(9)}$
}
\end{algorithm}
%
\noindent
